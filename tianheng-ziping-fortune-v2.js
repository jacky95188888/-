...
function deps(){var p=root.TianhengZipingPattern,f=root.TianhengZipingFlow,q=root.TianhengZipingQi,o=root.TianhengZipingOfficerKill;if(!p||!f||!q||!o)throw Error('需先載入 pattern、flow、qi 與 officer-kill 模組');return {p:p,f:f,q:q,o:o};}
...
function analyze(pillars,fortune,options){valid(pillars,fortune);var d=deps(),pt=d.p.analyzeFortune(pillars,fortune,options||{}),ft=d.f.analyzeFortune(pillars,fortune),qt=d.q.analyzeFortune(pillars,fortune),ot=d.o.analyzeFortune(pillars,fortune),qb=qt.before,qa=qt.after,rawTypes=pt.transition.types.slice(),types=rawTypes.slice(),qiEvent=null,conflicts=[];
 ...
 var dims=dimensionEvidence(pt);return {engine:'TianhengZipingFortune',version:'2.0-alpha',mode:'evidence-first',fortune:pt.fortune,original:{pattern:pt.base,flow:ft.before,qi:qb,officerKill:ot.before},afterFortune:{pattern:pt.after,flow:ft.after,qi:qa,officerKill:ot.after},transition:{types:types,rawTypes:rawTypes,pattern:pt.transition,flow:{changed:ft.flowChanged,deltaStrength:ft.deltaStrength},qi:qiEvent,conflictResolutions:conflicts},adviceEvidence:dims,adviceGenerated:false,legacyOverride:false,notes:['原局與運後資料分開保存','底層傷官見官證據不刪除，由官殺清純層記錄去留裁決','運程引動不等同事件必然發生']};}
