...
assert('農曆正月自動推得寅月春令', () => {
  const r = J.analyze({ question:'觀梅古例',category:'一般',askedAt:'2026-09-01T08:00:00+08:00',timezone:'Asia/Taipei',method:'lunar_time',calendar:{yearZhi:'辰',lunarMonth:1,lunarDay:1,hourZhi:'子',source:'固定測試'} });
...
assert('無效月支不是未判而是輸入錯誤', () => !J.safeAnalyze(input({upper:'乾',lower:'坎',movingLine:2},{monthZhi:'錯'})).ok);
assert('外應格式錯誤會攔截', () => !J.safeAnalyze(input({upper:'乾',lower:'坎',movingLine:2},{externalResponse:{note:''}})).ok);
