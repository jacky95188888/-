...
assert('三個形象古例皆完成從勢或歲運破象能力',()=>['DT-XIANG-004','DT-XIANG-005','DT-XIANG-006'].every(id=>B.run(id).comparison.status==='match'));
assert('源流運程轉化與戊午陽刃舊標註根因皆已修正',()=>B.run('DT-YUANLIU-007').comparison.status==='match'&&B.run('DT-YUANLIU-008').comparison.status==='match'&&B.run('DT-YUANLIU-008').engineSnapshot.basePattern==='陽刃格');
assert('官殺去留與跨格食神制殺完成',()=>B.run('DT-GUANSHA-009').comparison.status==='match'&&B.run('DT-SHISHA-010').comparison.status==='match');
assert('藏干歲運作用與用神強弱完成',()=>B.run('DT-SHANGGUAN-011').comparison.status==='match'&&B.run('DT-SHANGGUAN-012').comparison.status==='match');
const summary=B.summary();
assert('二十二例嚴格校準全部符合且無 partial mismatch',()=>summary.total===22&&summary.counts.match===22&&summary.counts.partial===0&&summary.counts.mismatch===0&&summary.notFullyMatched.length===0);
